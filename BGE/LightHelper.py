...
class LightManager():
	...
	def __init__(self):
		...
		self.template_lights = {
		'point':[],
		'sun':[],
		'spot':[],
		}
		self.existing_lights = {
		'point':[],
		'sun':[],
		'spot':[],
		}
...
def addLight(light_type = LT_POINT, time = 0, priority = 0):
	...
	if not hasattr(logic, 'lights_used'):
		...
		for l in [l for l in sce.objectsInactive if "DynLight" in l]: # Loop through the lamps that have the string "Dyn" + the type in their name (i.e. Point lamps named "Point", "Point.001", etc.)
			...
			logic.lights_used[l['DynLight']][l.name.lower()] = {'reference':None, 'priority':0} # And set their "used" status to 0

...
def Generic(particlelist):
	...
	for x in range(len(particles)):
		...
		try:
			...
			#if obj.invalid: # The equivalent of the try loop, but slower
...
def Flame(particlelist):
	...
	for x in range(len(particles)):
		...
		try: # If there's a SystemError (the particle doesn't exist anymore, but wasn't removed from the global particle list
			#if obj.invalid:
			#	particlelist.remove(obj)
			#	continue
